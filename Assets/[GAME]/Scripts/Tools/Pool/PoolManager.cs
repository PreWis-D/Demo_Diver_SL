...
public class PoolManager
{
    ...
    #region GetGenerics
    public static T GetPool<T>(T prefab, Vector3 pos, Quaternion quaternion, Transform parent) where T : Component
    {
    ...
    public static T GetPool<T>(T prefab, Transform parent) where T : Component
    {
        string name = prefab.gameObject.name;
        if (!_pools.ContainsKey(name))
            _pools[name] = new LinkedList<object>();
        ...
        if (_pools[name].Count > 0)
        {
            result = GetPoolFirst<T>(name, Vector3.zero, Quaternion.identity);
            return result;
        }
    ...
    public static GameObject GetPool(GameObject prefab, Transform parent)
    {
        string name = prefab.name;
        ...
        if (_pools[name].Count > 0)
        {
            result = GetPoolFirst(name, Vector3.zero, Quaternion.identity);
            return result;
        }
